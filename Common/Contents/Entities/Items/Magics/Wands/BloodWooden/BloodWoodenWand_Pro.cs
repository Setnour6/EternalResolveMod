...
using EternalResolve.Common.Contents.Entities.Buffs.Additions.LifeRegens;
...
{
    public class BloodWoodenWand_Pro : ERProjectile
    {
        public override void SetStaticDefaults( )
        {
            DisplayName.SetDefault( "血叶" );
        ...
        {
            Engine.AddBuff( Main.player[ Projectile.owner ] , ModContent.BuffType<LifeRegen_2>( ) , 120 );
        ...
        public override void AI( )
        {
            if ( Projectile.timeLeft == 120 )
            {
                for ( int angle = 0; angle < 12; angle++ )
                {
                    ...
                    Dust dust = Main.dust[ Dust.NewDust( position , 0 , 0 , 88 , 0f , 0f , 0 , Color.Red , 1.25f ) ];
        ...
        public override bool PreDraw( ref Color lightColor )
        {
            ...
            while ( i < 10 && !( Projectile.oldPos[ i ] == Vector2.Zero ) )
            {
                ...
                sizeFix2 /= ( 1 + i );
                sizeFix2 -= 1f;
                Color color = new Color( 255 , 22 , 64 );
